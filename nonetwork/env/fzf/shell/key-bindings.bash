...
__fzf_history__() (
  ...
  line=$(
    ...
    FZF_DEFAULT_OPTS="--height ${FZF_TMUX_HEIGHT:-40%} $FZF_DEFAULT_OPTS --tac --sync -n2..,.. --tiebreak=index --bind=ctrl-f:toggle-sort $FZF_CTRL_F_OPTS +m" $(__fzfcmd) |
    command grep '^ *[0-9]') &&
    if [[ $- =~ H ]]; then
      sed 's/^ *\([0-9]*\)\** .*/!\1/' <<< "$line"
    else
...
if [[ ! -o vi ]]; then
  ...
  bind '"\C-f": " \C-e\C-u\C-y\ey\C-u`__fzf_history__`\e\C-e\er\e^"'
  ...
  # CTRL-f - Paste the selected command from history into the command line
  bind '"\C-f": "\C-x\C-addi`__fzf_history__`\C-x\C-e\C-x\C-r\C-x^\C-x\C-a$a"'
